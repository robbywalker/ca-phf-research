...
namespace multicheck {
	template <class SYM>
	class multicheck_tabuSearcher : public tabu::TabuSearcher< multiTabuStatus<SYM>, arrays::Array<SYM> >
	{
		int * counts;
		int rows;
		int cols;
	...
	public:
		...
		multicheck_tabuSearcher( arrays::Array<SYM> & arr, int rows, int cols, int syms, int count, tabu::tabuStatus<SYM> ** _statuses, const char * stateFile, bool useFile ) : 
		  tabu::TabuSearcher< multiTabuStatus<SYM>, arrays::Array<SYM> >( multiTabuStatus<SYM>( _statuses, count, cols, 0 ), stateFile )
		{
			counts = new int[cols];
			this->rows = rows;
			this->cols = cols;
			...
			if ( useFile ) {
				initFromFile(arr.gen,stateFile);
			} else {
				current.reInit( arr, 0 );
				resetBest();
			}
